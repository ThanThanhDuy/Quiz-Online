<%-- 
    Document   : history
    Created on : Feb 6, 2021, 8:10:35 PM
    Author     : thant
--%>
...
<html>
	...
	<body>
		...
		<div style="display: flex;justify-content: center">
			<div  style="min-width: 1700px; height: 850px; padding: 15px;display: flex;justify-content: center;align-items: center">
				...
				<div class="shadow-lg" id="body-content" style="width: 100%;height: 750px;background-color: #fff;margin-bottom: 68px;border-radius: 10px">
					...
					<div class="row" style="padding: 20px;">
						<h2 style="text-align: center;margin: 0 auto; margin-bottom: 20px">History</h2>
						<div style="width: 100%;display: flex;justify-content: center;margin-bottom: 5px">
							<select class="form-control" style="width: 20%" onchange="location = this.value;">
								<c:forEach items="${sessionScope.LISTSUB}" var="listSub">
									<c:if test="${listSub.subId eq sessionScope.SUBID}">
										<option selected="true" value="View_History_Controller?sub=${listSub.subId}">${listSub.subId}</option>
									</c:if>
									<c:if test="${listSub.subId ne sessionScope.SUBID}">
										<option  value="View_History_Controller?sub=${listSub.subId}">${listSub.subId}</option>
									</c:if>
								</c:forEach>
							</select>
						</div>
						<div style="width: 100%;display: flex;justify-content: center;margin-bottom: 5px">
							<hr style="border-bottom: 5px solid #7dc129;width: 50%">
						</div>
						<div style="width: 100%;min-height: 510px;max-height: 510px;overflow-y: auto;margin-bottom: 10px;display: flex;justify-content: center;">
									...
									<tbody>
										...
										<c:forEach items="${requestScope.LIST}" var="list" varStatus="counter">
											<tr>
												...
												<td>${list.subId}</td>
												<td>${list.makeQuizDate}</td>
												<td>${list.mark}/10</td>
												<c:if test="${list.mark >=4}">
									...
									<c:if test="${sessionScope.SOPAGE>=2}">
										<c:forEach begin="1" end="${sessionScope.SOPAGE}" var="count">
											<c:if test="${count == requestScope.INDEX}">
												<li class="page-item active"><a class="page-link" href="View_History_Controller?index=${count}&sub=${sessionScope.SUBID}">${count}</a></li>
												</c:if>
												<c:if test="${count != requestScope.INDEX}">
												<li class="page-item"><a class="page-link" href="View_History_Controller?index=${count}&sub=${sessionScope.SUBID}">${count}</a></li>
												</c:if>
											</c:forEach>
										</c:if>
								</ul>
		...
		<div style="display: flex;justify-content: center">
			<menu class="menu" style="border-radius: 10px">
				...
				<div style="margin: 30px;margin-top: 0px;margin-bottom: 0px;">
					<a href="Show_Sub_Home_Controller" class="menu__item" style="--bgColorItem: #e0b115;">
				...
				<div style="margin: 30px;margin-top: 0px;margin-bottom: 0px;">
					<a href="View_History_Controller" class="menu__item active" style="--bgColorItem: #f54888;">
						<svg class="icon" viewBox="0 0 24 24">
