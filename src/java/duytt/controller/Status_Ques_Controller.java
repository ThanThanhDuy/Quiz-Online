 ...
 * @author thant
 */
public class Status_Ques_Controller extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (status.isEmpty() || status == null || status.equals("")) {
				check = false;
				request.setAttribute("STATUS_ERROR", "Please Choose again STATUS");
			} else if (!status.matches("true") && !status.matches("false")) {
				check = false;
				request.setAttribute("STATUS_ERROR", "Please Choose again STATUS");
			}
			List<Subject> listSub = new SubjectDAO().getSubject();
			if (check) {
				List<QuesSearch> list=new QuestionDAO().getDataStatus(Integer.parseInt(index), status);				
				int soluong = new QuestionDAO().countQuesStatus(status);
				...
				session.setAttribute("CONTROLLER", "Status_Ques_Controller");
			}

			session.setAttribute("LISTSUB", listSub);

		} catch (Exception e) {
			log("Status_Ques_Controller: " + e.toString());
		} finally {
